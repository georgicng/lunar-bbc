    ...
    <template>
        <div className="md:p-10 p-4 space-y-4">
            <h2 className="text-lg font-medium">Orders List</h2>
                <div v-for="(order, index) in orders" :key="index" className="flex flex-col md:grid md:grid-cols-[2fr_1fr_1fr_1fr] md:items-center gap-5 p-5 max-w-4xl rounded-md border border-gray-300 text-gray-800">
                    <div className="flex gap-5">
                        <img className="w-12 h-12 object-cover opacity-60" :src="boxIcon" alt="boxIcon" />
                                <div v-for="(item, _index) in order.items" :key="_index" className="flex flex-col justify-center">
                                    <p className="font-medium">
                                        {{item.product}} <span :class="[`text-indigo-500 ${item.quantity < 2 && 'hidden'}`]">x {{item.quantity}}</span>
                                    </p>
                                </div>
                    </div>
